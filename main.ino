  ...
  if (Serial.available() > 0) {
    ...
    if (commandCount > 0) {
      ...
      for (int i = 0; i < commandCount - 1; i++) {
        ...
        functionParameters[i] = parsedCommands[i + 1].toFloat();
...
void handleMoveAcceleration(){
  ...
  if(joint == 1){
    ...
    float desiredAcc = omega/(beta1/omega);
    articulacion2.motor1.setAcceleration(desiredAcc*5);
    articulacion2.motor1.moveTo(alpha);
    articulacion2.motor2.setAcceleration(desiredAcc*5);
    ...
    if(alpha < articulacion2.motor1.currentPosition()){
      ...
      articulacion2.motor2.setSpeed(omega);
      ...
      articulacion2.motor1.setSpeed(omega);
      articulacion2.motor2.setSpeed(omega);
      //articulacion2.motor1.setSpeed(articulacion2.motor1.speed());
      //articulacion2.motor2.setSpeed(articulacion2.motor2.speed());
      while (abs(articulacion2.motor1.currentPosition()) <= beta2){
        articulacion2.motor1.runSpeedToPosition();
        articulacion2.motor2.runSpeedToPosition();
      }
...
void handleGoPose(){
  ...
  while(!((articulacion1.motor1.distanceToGo() == 0)&&(articulacion2.motor1.distanceToGo() == 0)&&(articulacion3.motor1.distanceToGo() == 0)&&(articulacion4.motor1.distanceToGo() == 0)&&(articulacion5.motor1.distanceToGo() == 0))){
    articulacion1.motor1.runSpeedToPosition();
    articulacion2.motor1.runSpeedToPosition();
    articulacion2.motor2.runSpeedToPosition();
    articulacion3.motor1.runSpeedToPosition();
...
void handleSetTrajectory(){
  ...
  for(int i=0;i<(trajectorycount);i++){
    ...
    Serial.print(functionParameters[i*3]);
    Serial.print(" ");
    Serial.print(functionParameters[i*3+1]);
    Serial.print(" ");
    Serial.println(functionParameters[i*3+2]);;
...
void handleGoTrajectory(){
  ...
  for(int i = 0; i<trajectorycount;i++){
    ...
    while(!((abs(articulacion1.motor1.distanceToGo() == 0))&&(articulacion2.motor1.distanceToGo() == 0)&&(articulacion3.motor1.distanceToGo() == 0))){
      articulacion1.motor1.runSpeedToPosition();
      articulacion2.motor1.runSpeedToPosition();
      articulacion2.motor2.runSpeedToPosition();
      articulacion3.motor1.runSpeedToPosition();
